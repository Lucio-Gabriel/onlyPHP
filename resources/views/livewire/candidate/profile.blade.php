        ...
        <div class="bg-background font-sans">
            <main class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <section class="mb-8">
                        ...
                        <div class="bg-card rounded-xl border border-border p-6 shadow-sm mb-4">
                            <div class="flex items-center space-x-4 mb-4">
                                @if ($profile->profile_picture_path)
                                    <img src="{{ Storage::disk('s3')->url($profile->profile_picture_path) }}"
                                        alt="Foto de Perfil"
                                        class="w-24 h-24 rounded-full object-cover border border-border">
                                @else
                                    <div
                                        class="w-24 h-24 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 text-4xl">
                            ...
                            <div class="space-y-2">
                                <p class="text-foreground"><span class="font-bold">Stack Principal:</span>
                                    {{ implode(', ', (array) $profile->main_stack) }}</p>
                                @if ($profile->github_link)
                                    <p class="text-foreground"><span class="font-bold">GitHub:</span> <a
                                            href="{{ $profile->github_link }}" target="_blank"
                                            class="text-blue-500 hover:underline">{{ $profile->github_link }}</a></p>
                                @endif
                                @if ($profile->portfolio_link)
                                    <p class="text-foreground"><span class="font-bold">Portfólio:</span> <a
                                            href="{{ $profile->portfolio_link }}" target="_blank"
                                            class="text-blue-500 hover:underline">{{ $profile->portfolio_link }}</a></p>
                                @endif
                                @if ($profile->linkedin_link)
                                    <p class="text-foreground"><span class="font-bold">LinkedIn:</span> <a
                                            href="{{ $profile->linkedin_link }}" target="_blank"
                                            class="text-blue-500 hover:underline">{{ $profile->linkedin_link }}</a></p>
                                @endif
                                @if ($profile->resume_url)
                                    <p class="text-foreground flex items-center"><span class="font-bold flex items-center mr-2">
                                            <x-svg.curriculum class="w-5 h-5 inline-block" />
                                            Currículo:</span> <a href="{{ $profile->resume_url }}" target="_blank"
                    ...
                    @else
                        <p class="text-gray-600 mb-4">Nenhum perfil encontrado. Preencha os dados abaixo para criar um.
                        </p>
                    @endif
                </section>
                ...
                <form wire:submit.prevent="updateProfile"
                    class="bg-card shadow-sm rounded-xl border border-border p-6 mb-4">
                    <h2 class="text-2xl font-bold mb-6 text-foreground">Editar Dados do Perfil</h2>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            <label for="full_name" class="block text-foreground text-sm font-bold mb-2">Nome
                                Completo:</label>
                    ...
                    <div class="mb-4">
                        <label for="main_stack" class="block text-foreground text-sm font-bold mb-2">Stack Principal
                            (selecione uma ou mais):</label>
                    ...
                    <div class="mb-4">
                        <label for="github_link" class="block text-foreground text-sm font-bold mb-2">Link do
                            GitHub:</label>
                    ...
                    <div class="mb-4">
                        <label for="portfolio_link" class="block text-foreground text-sm font-bold mb-2">Link do
                            Portfólio:</label>
                        <input type="url" id="portfolio_link" wire:model="portfolio_link"
                    ...
                    <div class="mb-4">
                        <label for="linkedin_link" class="block text-foreground text-sm font-bold mb-2">Link do
                            LinkedIn:</label>
                        <input type="url" id="linkedin_link" wire:model="linkedin_link"
                ...
                <form wire:submit.prevent="uploadResume"
                    ...
                    <div class="mb-4">
                        ...
                        <div wire:loading wire:target="resumeFile" class="text-blue-500 text-sm mt-2">Carregando
                            arquivo...</div>
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                        <button type="submit"
                            ...
                            wire:loading.attr="disabled" wire:target="uploadResume">
                ...
                <form wire:submit.prevent="uploadProfilePicture"
                    ...
                    <div class="mb-4">
                        ...
                        @if ($profilePictureFile && method_exists($profilePictureFile, 'isPreviewable') && $profilePictureFile->isPreviewable())
                            <div class="mt-4">
                                ...
                                <img src="{{ $profilePictureFile->temporaryUrl() }}"
                                    class="w-24 h-24 rounded-full object-cover border border-border">
                            </div>
                        ...
                        <div wire:loading wire:target="profilePictureFile" class="text-blue-500 text-sm mt-2">
                            Carregando foto...</div>
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                        <button type="submit"
                            ...
                            wire:loading.attr="disabled" wire:target="uploadProfilePicture">
